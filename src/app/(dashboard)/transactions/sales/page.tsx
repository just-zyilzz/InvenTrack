...
import { ShoppingCart, Plus, FileSpreadsheet, FileText } from "lucide-react";
...
export default function SalesPage() {
    ...
    const fetchTransactions = async () => {
        try {
            const res = await fetch("/api/transactions?type=SALE");
    ...
    const handleDownloadExcel = async () => {
        const res = await fetch("/api/export/sales?type=SALE");
    ...
    const handleDownloadPdf = () => {
        exportTransactionsPdf(transactions, "SALE");
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Penjualan</h1>
